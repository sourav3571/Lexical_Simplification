...
from bert_ranker import GatedFusionRanker
...
class AILexicalSimplifier:
    ...
    def __init__(self, config: Dict[str, Any], device: torch.device = None) -> None:
        ...
        # Pipeline components
        self.surprisal_calc    = BERTSurprisalCalculator(self.tokenizer, self.mlm_model, self.device)
        self.complexity_scorer = BERTComplexityScorer(self.tokenizer, self.mlm_model, self.bert_model, self.device)
        self.cwi               = DynamicContextualCWI(self.tokenizer, self.mlm_model, self.bert_model, self.device)
        self.disambiguator     = BERTSenseDisambiguator(self.tokenizer, self.bert_model, self.device)
        self.cand_gen          = BERTCandidateGenerator(self.tokenizer, self.mlm_model, self.bert_model, self.device)
        self.validator         = BERTValidator(self.tokenizer, self.mlm_model, self.bert_model, self.device)
        self.replacer          = ParallelReplacer()

        # Neural ranker
        self.ranker     = GatedFusionRanker()
        ranker_path     = "gated_fusion_ranker.pt"
        if os.path.exists(ranker_path):
            try:
                self.ranker.load_state_dict(
                    torch.load(ranker_path, map_location=self.device))
                print(f"Loaded trained GatedFusionRanker weights from {ranker_path}.")
            except Exception as e:
                print(f"Could not load GatedFusionRanker weights: {e}. Using default weights.")
    ...
    def simplify(self, sentence: str, verbose: bool = True) -> str:
        ...
        if verbose:
            print(f"\nSTAGE 2 - CWI  (threshold = {CWI_THRESHOLD}):")
            print(f"  {'word':15} {'score':8} {'zipf_freq':10} {'decision':16} reason")
            print("  " + "-" * 72)
        ...
        for res in cwi_results:
            ...
            freq     = wordfreq.zipf_frequency(w.lower(), 'en')
            decision = "COMPLEX [FAIL]" if is_comp else "SIMPLE  [PASS]"
            reason   = (f"score {score:.3f} >= {CWI_THRESHOLD}" if is_comp
                        else f"score {score:.3f} <  {CWI_THRESHOLD}")
            if verbose:
                print(f"  {w:15} {score:8.4f} {freq:10.2f}  {decision}  {reason}")
            if is_comp:
        ...
        for cw in complex_words:
            ...
            sources = self.cand_gen.generate_raw_candidates_by_source(
                sentence, word, start_char, end_char, chosen_sense)
            ...
            if verbose:
                print(f"\n  Filter detail "
                      f"(need freq_gain>={FREQ_GAIN_MIN}, "
                      f"cand_freq>={CAND_FREQ_MIN}, "
                      f"mlm_prob>={MLM_PROB_MIN}, "
                      f"semantic_sim>=0.85):")
                print(f"  {'candidate':15} {'orig_f':7} {'cand_f':7} "
                      f"{'gain':7} {'f_ok':5} {'mlm_p':9} {'m_ok':5} {'sem':5} verdict")
                print("  " + "-" * 82)
            ...
            for cand in sorted(raw_pool):
                ...
                passes_freq = (freq_gain >= FREQ_GAIN_MIN) and (cand_freq >= CAND_FREQ_MIN)
                passes_mlm  = mlm_prob >= MLM_PROB_MIN
                passes_sem  = semantic_sim >= 0.85
                accepted    = passes_freq and passes_mlm and passes_sem

                if verbose:
                    reject_reason = ""
                    if not passes_freq:
                        if freq_gain < FREQ_GAIN_MIN:
                            reject_reason = f"gain {freq_gain:.2f} < {FREQ_GAIN_MIN}"
                        elif cand_freq < CAND_FREQ_MIN:
                            reject_reason = f"freq {cand_freq:.2f} < {CAND_FREQ_MIN}"
                    elif not passes_mlm:
                        reject_reason = f"mlm {mlm_prob:.4f} < {MLM_PROB_MIN}"
                    elif not passes_sem:
                        reject_reason = f"sem {semantic_sim:.4f} < 0.85"
                    ...
                    print(f"  {cand:15} {orig_zipf:7.2f} {cand_freq:7.2f} "
                          ...
                          f"{'yes' if passes_mlm else 'no':5} {semantic_sim:5.2f} {verdict}")
            ...
            for cand in filtered_cands:
                ...
                rank_score     = self.ranker.predict(mlm_prob, cosine_sim,
                                                     surp_red, fluency_change)
                rank_score    += 0.50 * sense_sim
                rank_score    += 0.10 * sentence_sim
                semantic_priority = sense_sim + 0.50 * mlm_prob
            ...
            scored_candidates.sort(key=lambda x: (x['semantic_priority'], x['sense_sim'], x['rank_score']), reverse=True)

            if verbose:
                print(f"  {'candidate':15} {'mlm_p':8} {'cos_sim':8} {'sent_sim':8} {'sense_sim':8} {'sem_pri':8} {'surp_red':9} {'fluency':8} {'score':8}")
                print("  " + "-" * 102)
                for i, sc in enumerate(scored_candidates):
                    ...
                    print(f"  {sc['candidate']:15} {sc['mlm_prob']:8.4f} "
                          ...
                          f"{sc['sense_sim']:8.4f} {sc['semantic_priority']:8.4f} "
                          f"{sc['surp_red']:9.4f} {sc['fluency_change']:8.4f} {sc['rank_score']:8.4f}{marker}")
